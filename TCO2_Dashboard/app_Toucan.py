...
#Figures
#7-day-performance
fig_seven_day_volume = sub_plots_volume(sd_pool,last_sd_pool,title_indicator="Credits Bridged (7d)",title_graph="")
fig_seven_day_volume_retired = sub_plots_volume(sd_pool_retired,last_sd_pool_retired,"Credits Retired (7d)","")
fig_seven_day_vintage = sub_plots_vintage(sd_pool,last_sd_pool,"Average Credit Vintage (7d)","")
fig_seven_day_vintage_retired = sub_plots_vintage(sd_pool_retired,last_sd_pool_retired,"Average Credit Vintage (7d)","")
...
#30-day-performance
fig_thirty_day_volume  = sub_plots_volume(td_pool,last_td_pool,"Credits Bridged (30d)","")
fig_thirty_day_volume_retired = sub_plots_volume(td_pool_retired,last_td_pool_retired,"Credits Retired (30d)","")
fig_thirty_day_vintage = sub_plots_vintage(td_pool,last_td_pool,"Average Credit Vintage (7d)","")
fig_thirty_day_vintage_retired = sub_plots_vintage(td_pool_retired,last_td_pool_retired,"Average Credit Vintage (30d)","")
...
content_Toucan=[
  dbc.Row(
        dbc.Col(
            dbc.Card([
                    dbc.CardHeader(html.H1("Toucan Carbon Credits Dashboard",className='page-title'))
            ])
        ,width=8)
    ),
  ...
  dbc.Row([
    dbc.Col(dbc.Card([
            ...
            dbc.CardBody("{:,}".format(int(df["Quantity"].sum())),className="card-text")
    ])
    ,width=4),
    dbc.Col(dbc.Card([
            html.H5("Retired Tokenized Credits",className="card-title"),
            dbc.CardBody("{:,}".format(int(df_retired["Quantity"].sum())),className="card-text")
    ])
    ,width=4),
    dbc.Col(dbc.Card([
            html.H5("Current Supply of Tokenized Credits",className="card-title"),
            dbc.CardBody("{:,}".format(int(df["Quantity"].sum()-df_retired["Quantity"].sum())),className="card-text")
    ])
    ,width=4),
  ...
  dbc.Row([
    ...
    dbc.Col([
      dbc.Card([
        html.H5("Conversion to Eligible Ratio of a specific Carbon Pool",className="card-title"),
        dbc.CardBody([dbc.Row([
          dbc.Col(
          dcc.Dropdown(options=[{'label':'BCT','value':'BCT'},
          {'label':'NCT','value':'NCT'}],value='BCT',id='pie_chart_summary',placeholder='Select Carbon Pool',style=dict(font=dict(color='black'))),width=2)
    ...
    dbc.Card([
      ...
      dbc.CardBody([
        dbc.Row([
          dbc.Col([
            dbc.Card([
              dbc.CardHeader(html.H5("Select Performance Range",className="card-title"),),
        ...
        {'label':'Overall Performance','value':'Overall Performance'}],value='Overall Performance',id='summary_type',placeholder='Select Summary Type')])
          ...
          dbc.Col([
            dbc.Card([
              dbc.CardHeader(html.H5("Bridged or Retired TCO2s?",className="card-title"),),
  ...
  dbc.Row([
    dbc.Col(),
    dbc.Col(dbc.Card([
      dbc.CardBody(html.H2(id="Last X Days"),style ={'textAlign': 'center'})
    ]),width=6),
    dbc.Col(),
  ]),
...
#         return "Last 7 Days Performance",fig_seven_day_volume,fig_seven_day_vintage,fig_seven_day_map,html.Br()
#       elif TCO2_type == 'Retired':
#         return "Last 7 Days Performance",fig_seven_day_volume_retired,fig_seven_day_vintage_retired,fig_seven_day_map_retired,html.Br()
    
#     elif summary_type=='Last 30 Days Performance':
#       if TCO2_type == 'Bridged':
#         return "Last 30 Days Performance",fig_thirty_day_volume,fig_thirty_day_vintage,fig_thirty_day_map,html.Br()
#       elif TCO2_type == 'Retired':
#         return "Last 30 Days Performance",fig_thirty_day_volume_retired,fig_thirty_day_vintage_retired,fig_thirty_day_map_retired,html.Br()
