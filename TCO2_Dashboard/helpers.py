...
def date_manipulations(df):
    df["Date"] =pd.to_datetime(df["Date"],unit='s').dt.tz_localize(None).dt.floor('D').dt.date
...
def subsets(df):
    ...
    sd_pool =df[(df["Date"] < current_time.date()) &(df["Date"] >= seven_day_start.date())]
    last_sd_pool =df[(df["Date"] < seven_day_start.date()) &(df["Date"] >= last_seven_day_start.date())]
    # # Thirty day pool subsets
    td_pool =df[(df["Date"] < current_time.date()) &(df["Date"] >= thirty_day_start.date())]
    last_td_pool =df[(df["Date"] < thirty_day_start.date()) &(df["Date"] >= last_thirty_day_start.date())]

    return sd_pool, last_sd_pool, td_pool, last_td_pool

def filter_carbon_pool(df_deposited,df_redeemed,tokenadddress):
  df_redeemed = df_redeemed[df_redeemed["Pool"]==tokenadddress].reset_index()
  df_deposited = df_deposited[df_deposited["Pool"]==tokenadddress].reset_index()
